...
static void on_paint(window_t *win) {
  ...
  gfx_fill_circle_buffer(win->buffer, win->width, win->width / 2, 80, 40,
                         0xFFFFD700); /* Sun */

  font_draw_string_buffer(win->buffer, win->width, win->width / 2 - 30, 140,
                          "22°C", COLOR_TEXT_DARK, 0);
  font_draw_string_buffer(win->buffer, win->width, win->width / 2 - 40, 170,
                          "Sunny Sky", RGBA(100, 100, 120, 255), 0);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    font_draw_string_buffer(win->buffer, win->width, 30 + i * 60, 220, "Mon",
                            COLOR_TEXT_DARK, 0);
  }
}
