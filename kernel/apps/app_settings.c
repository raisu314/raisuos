...
static void on_paint(window_t *win) {
  ...
  int y = 20, x = 20;
  font_draw_string_buffer(win->buffer, win->width, win->height, x, y, "ADNWS Settings",
                          RGBA(0, 100, 200, 255), 0);
  y += 40;

  /* Sidebar / Options */
  font_draw_string_buffer(win->buffer, win->width, win->height, 20, 50, "> Display & Ratios",
                          COLOR_TEXT_DARK, 0);
  font_draw_string_buffer(win->buffer, win->width, win->height, 20, 80,
                          "> Raisu-Glass Theme", COLOR_TEXT_DARK, 0);
  font_draw_string_buffer(win->buffer, win->width, win->height, 20, 110, "> Language (i18n)",
                          COLOR_TEXT_DARK, 0);
  ...
  font_draw_string_buffer(win->buffer, win->width, win->height, 30, 160,
                          "Subsystem: ADNWS v1.0.0", COLOR_SUB, 0);
  font_draw_string_buffer(win->buffer, win->width, win->height, 30, 180,
                          "Build: 2026.02.28-v1.0.0", RGBA(150, 150, 160, 255),
                          0);
  font_draw_string_buffer(win->buffer, win->width, win->height, 30, 200,
